...
class MIMICDataLoader:
    ...
    def load_patients(self):
        return pd.read_csv(f"{self.hosp_path}patients.csv")
    ...
    def load_chartevents(self, itemids=None, stay_ids=None, chunksize=100000):
        ...
        for i, chunk in enumerate(pd.read_csv(
            f"{self.icu_path}chartevents.csv", chunksize=chunksize, low_memory=False
        )):
    ...
    def load_labevents(self, itemids=None, subject_ids=None, chunksize=100000):
        ...
        # Fix: specify dtype for mixed-type column to suppress DtypeWarning
        for i, chunk in enumerate(pd.read_csv(
            f"{self.hosp_path}labevents.csv",
            chunksize=chunksize,
            low_memory=False,
            dtype={"order_provider_id": str},
        )):
            if itemids is not None:
                chunk = chunk[chunk["itemid"].isin(itemids)]
    ...
    def load_inputevents(self):
        inputevents = pd.read_csv(f"{self.icu_path}inputevents.csv", low_memory=False)
    ...
    def load_outputevents(self):
        outputevents = pd.read_csv(f"{self.icu_path}outputevents.csv", low_memory=False)
        outputevents["charttime"] = pd.to_datetime(outputevents["charttime"])
        return outputevents

    def load_d_items(self):
        return pd.read_csv(f"{self.icu_path}d_items.csv")

    def load_d_labitems(self):
        return pd.read_csv(f"{self.hosp_path}d_labitems.csv")